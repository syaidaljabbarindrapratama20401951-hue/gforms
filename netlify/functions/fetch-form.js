...
function normaliseConfig(value={}){return{duration:clampNumber(value.duration,0,600,0),sessionName:clean(value.sessionName,80),subject:clean(value.subject,100),className:clean(value.className,80),examDate:/^\d{4}-\d{2}-\d{2}$/.test(value.examDate||'')?value.examDate:'',token:clean(value.token,40),theme:['exam','business','casual'].includes(value.theme)?value.theme:'exam',themeMode:value.themeMode==='dark'?'dark':'light',soundWarnings:Boolean(value.soundWarnings),shuffleOptions:Boolean(value.shuffleOptions),violationsEnabled:value.violationsEnabled!==false,violationLimit:clampNumber(value.violationLimit,1,10,3),instructions:clean(value.instructions,1200),identityEntryIds:[]}}
...
exports._test={parseGoogleForm,extractFormId,normaliseConfig,extractSubmitMetadata,decodeExamId,encodeExamId,canonicalViewUrl,fetchHtml,userError};
